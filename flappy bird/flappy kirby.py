...
backround = pygame.image.load('images/flappy bird bg.png')
ground = pygame.image.load('images/flappy bird ground.png')
...
def reset_game():
    global score, game_over, flying, made_button, animate, restart_button
    chicken.v = 0
...
class Button:
    ...
    def __init__(self,x,y):
        ...
        self.scale = pygame.transform.scale(self.image,(250,80))
...
while running:
    ...
    if game_over == True:
        if made_button == False:
            restart_button = Button(WIDTH/2-125,HEIGHT/2-100)
            made_button = True
        restart_button.draw()
    if flying == True:
        if time - last_pipe > pipe_frequency:
            pipe_height = randint(-150,150)-84
